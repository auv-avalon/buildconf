...
if [ "$ANSWER" = "n" ]; then
    $RUBY autoproj_bootstrap $@ git http://$CONF_SERVER/$CONF_REPO push_to=git@$CONF_SERVER:$CONF_REPO branch=asv
else
    $RUBY autoproj_bootstrap $@ git git://$CONF_SERVER/$CONF_REPO push_to=git@$CONF_SERVER:$CONF_REPO branch=asv
fi
